...
class ansatz:
    ...
    def get_scores(self, inds_chk):
        ...
        for i, ind in enumerate(inds_chk):
            if i%self._msize == self._mrank:
                scores[ind], _, _, _, _ = self.get_score([ind])
        ...
        for _ in range(self._max_add):
            ...
            for i, idx in enumerate(isort):
                ...
                if idx in ichoose or \
                    numpy.any([self._pool[idx][1][iq] != "I" for iq in qsupport]):
                    continue
                ichoose.append(idx)
                inds_chk.remove(idx)
            ...
            diff, mmat, vvec, vecp_adds, dthdt = self.get_score(ichoose)
    ...
    def save_ansatz(self):
        with h5py.File("data/ansatz"+self.filename+".h5", "w") as f:
    ...
    def psave_ansatz(self):
        with open("data/ansatz"+self.filename+".pkle", "wb") as f:
            data = [self._ansatz,
                    self._params,
                    self._ref_state,
                    self._ngates,
    ...
    def psave_ansatz_simp(self):
        # for continuous run
        with open("data/ansatz_simp"+self.filename+".pkle", "wb") as f:
    ...
    def psave_ansatz_inp(self):
        ...
        with open("data/ansatz_inp"+self.filename+".pkle", "wb") as f:
            data = [self._ansatz[1],
                    self._params,
                    ]
            pickle.dump(data, f)
